...
describe("onMessage", () => {
  ...
  it("responds to a regular WCL link", async () => {
    ...
    expect(message.channel.send).toHaveBeenCalledWith(
      "https://wowanalyzer.com/report/PROPERREPORTCODE"
    );
  });
  it("responds to a message with padding text", async () => {
    const message = createMessage({
      content:
        "Please review: https://www.warcraftlogs.com/reports/PROPERREPORTCODE. Thanks!",
    });
  ...
  it("does not respond to a message with an invalid or private report", async () => {
    const message = createMessage({
      content:
        "Please review: https://www.warcraftlogs.com/reports/AWRONGREPORTCODE. Thanks!",
    });
  ...
  it("ignores messages with multiple links", async () => {
    const message = createMessage({
      content:
        "https://www.warcraftlogs.com/reports/AB1CDEf2G3HIjk4L and https://www.warcraftlogs.com/reports/aaaaaaaaaaaaaaaa",
    });
  ...
  it("preselects fight and player", async () => {
    const message = createMessage({
      content:
        "https://www.warcraftlogs.com/reports/PROPERREPORTCODE/#fight=11&source=75&type=healing",
    });

    await onMessage(client, message);

    expect(message.channel.send).toHaveBeenCalledWith(
      "https://wowanalyzer.com/report/PROPERREPORTCODE/11-Mythic+Sisters+of+the+Moon+-+Kill+(5:16)/75-Zerotorescue"
    );
  });
  it("preselects fight when it's alone", async () => {
    const message = createMessage({
      content:
        "https://www.warcraftlogs.com/reports/PROPERREPORTCODE/#fight=11",
    });

    await onMessage(client, message);

    expect(message.channel.send).toHaveBeenCalledWith(
      "https://wowanalyzer.com/report/PROPERREPORTCODE/11-Mythic+Sisters+of+the+Moon+-+Kill+(5:16)"
    );
  });
  it("preselects fight when it's last", async () => {
    const message = createMessage({
      content:
        "https://www.warcraftlogs.com/reports/PROPERREPORTCODE/#fight=last",
    });

    await onMessage(client, message);

    expect(message.channel.send).toHaveBeenCalledWith(
      "https://wowanalyzer.com/report/PROPERREPORTCODE/42-Mythic+Fallen+Avatar+-+Wipe+14+(3:49)"
    );
  });
  it("ignores preselected player if fight isn't provided", async () => {
    const message = createMessage({
      content:
        "https://www.warcraftlogs.com/reports/PROPERREPORTCODE/#source=75&type=healing",
    });
  ...
  it("ignores reports with advanced filters", async () => {
    const message1 = createMessage({
      content:
        "https://www.warcraftlogs.com/reports/PROPERREPORTCODE/#source=75&type=healing&start=2356367&end=2381131",
    });

    await onMessage(client, message1);

    expect(message1.channel.send).not.toHaveBeenCalled();

    const message2 = createMessage({
      content:
        "https://www.warcraftlogs.com/reports/PROPERREPORTCODE/#fight=11&source=75&start=2356367&end=2381131&view=events&pins=2%24Off%24%23244F4B%24auras-gained%24-1%240.0.0.Any%240.0.0.Any%24true%240.0.0.Any%24true%2431821%24true%24true",
    });

    await onMessage(client, message2);

    expect(message2.channel.send).not.toHaveBeenCalled();

    const message3 = createMessage({
      content:
        "https://www.warcraftlogs.com/reports/PROPERREPORTCODE/#source=75&fight=last&type=healing&phase=1",
    });

    await onMessage(client, message3);
  ...
  it("should not respond to itself", async () => {
    const message = createMessage({
      embeds: [
        {
          url: "https://www.warcraftlogs.com/reports/PROPERREPORTCODE",
